...
public class SignUpPageValidation {
	...
	public void testTaxinearu() throws Exception {
		...
		// started passing the values from excel validation value 1
		Function.signUp(driver, 1);

		Log.info("VALIDATION 1 VALUES PASSED SUCCESSFULLY");
		Thread.sleep(2000);

		// checking for the errors present after passing the values
		...
		driver.findElement(By.id("altEmailId")).clear();
		...
		// started passing the values from excel validation value 2
		...
		// started passing the values from excel validation value 3
		...
		if (driver.getPageSource().contains(Constant.OLD_EMAIL)) {
			Log.info("EMAIL ALREADY EXISTT,SO WE HAVE TO ENTER A NEW EMAIL ERROR IS PRESENT");
			ExcelUtils.setCellData("PASS", 6, 12, Constant.SHEET_NAME4);
		} else {
			Log.info("EMAIL ALREADY EXISTT,SO WE HAVE TO ENTER A NEW EMAIL ERROR IS NOT PRESENT");
